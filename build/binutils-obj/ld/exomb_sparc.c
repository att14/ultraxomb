...
/* 64 bit ELF emulation code for xomb_sparc
...
static void gldxomb_sparc_before_parse (void);
static void gldxomb_sparc_after_open (void);
static void gldxomb_sparc_before_allocation (void);
static void gldxomb_sparc_after_allocation (void);
static lang_output_section_statement_type *gldxomb_sparc_place_orphan
  (asection *, const char *, int);

static void
gldxomb_sparc_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldxomb_sparc_before_parse (void)
{
  ldfile_set_output_arch ("sparc:v9", bfd_arch_sparc);
...
static bfd_boolean
gldxomb_sparc_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldxomb_sparc_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldxomb_sparc_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldxomb_sparc_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldxomb_sparc_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldxomb_sparc_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldxomb_sparc_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldxomb_sparc_try_needed (n, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldxomb_sparc_try_needed (&needed, force))
	return TRUE;
...
static void
gldxomb_sparc_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldxomb_sparc_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldxomb_sparc_write_build_id_section (bfd *abfd)
{
...
gldxomb_sparc_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldxomb_sparc_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldxomb_sparc_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldxomb_sparc_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldxomb_sparc_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldxomb_sparc_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldxomb_sparc_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldxomb_sparc_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldxomb_sparc_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldxomb_sparc_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldxomb_sparc_find_exp_assignment (exp->binary.lhs);
      gldxomb_sparc_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldxomb_sparc_find_exp_assignment (exp->trinary.cond);
      gldxomb_sparc_find_exp_assignment (exp->trinary.lhs);
      gldxomb_sparc_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldxomb_sparc_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldxomb_sparc_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldxomb_sparc_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void 
gldxomb_sparc_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
static void
gldxomb_sparc_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  bfd *abfd;
  ...
  lang_for_each_statement (gldxomb_sparc_find_statement_assignment);
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldxomb_sparc_append_to_separated_string (&depaudit, cp);
...
static bfd_boolean
gldxomb_sparc_open_dynamic_archive
...
static lang_output_section_statement_type *
gldxomb_sparc_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
static void
gldxomb_sparc_after_allocation (void)
{
  bfd_boolean need_layout = bfd_elf_discard_info (link_info.output_bfd,
						  &link_info);
  gldxomb_sparc_map_segments (need_layout);
}

static char *
gldxomb_sparc_get_script (int *isfile)
{
  *isfile = 0;
...
; }
#define gldxomb_sparc_add_options NULL
#define gldxomb_sparc_handle_option NULL
#define gldxomb_sparc_list_options NULL

struct ld_emulation_xfer_struct ld_xomb_sparc_emulation =
{
  gldxomb_sparc_before_parse,
  syslib_default,
  hll_default,
  after_parse_default,
  gldxomb_sparc_after_open,
  gldxomb_sparc_after_allocation,
  set_output_arch_default,
  ldemul_default_target,
  gldxomb_sparc_before_allocation,
  gldxomb_sparc_get_script,
  "xomb_sparc",
  "elf64-sparc",
  finish_default,
  NULL,
  gldxomb_sparc_open_dynamic_archive,
  gldxomb_sparc_place_orphan,
  NULL,
  NULL,
  gldxomb_sparc_add_options,
  gldxomb_sparc_handle_option,
  NULL,
  gldxomb_sparc_list_options,
  gldxomb_sparc_load_symbols,
  NULL,
  NULL
};
